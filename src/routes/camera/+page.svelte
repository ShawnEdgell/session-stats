<script lang="ts">
	...
	interface CameraSetting {
		...
		description: string;
		created_at?: Date;
		discord_id?: string; // Add discord_id to CameraSetting interface
	...
	async function handleSubmit() {
		...
		if (error) {
			...
			title = '';
			description = '';
			fetchCameraSettings(); // Refresh camera settings after successful submission
		}
	}
	...
	async function deletePost(id: number) {
		try {
			...
			if (error) {
				...
				fetchCameraSettings(); // Refresh camera settings after successful deletion
			}
		} catch (error: any) {
			console.error(`Error deleting post: ${error.message}`);
		}
	...
	async function updatePost(setting: CameraSetting) {
		try {
			...
			if (error) {
				...
				fetchCameraSettings(); // Refresh camera settings after successful update
			}
		} catch (error: any) {
			console.error(`Error updating post: ${error.message}`);
		}
